...
이 문제는 DFS 알고리즘을 이용해서 풀 수 있었다. 문제를 제대로 이해한다면 크게 어렵지 않게 풀 수 있다.
...
public class Main {
    ...
    public static void dfs(int[] a, int idx) {
        if(finish) return;
    ...
    public static boolean check(int[] a, int length) {
        for(int i=0; i<length; i++) {
            int x = 0;
            for(int j=0; j<length-i; j++) {
                x+=a[i+j];
                if(x>0 && map[i][i+j]!='+') return false;
                if(x==0 && map[i][i+j]!='0') return false;
                if(x<0 && map[i][i+j]!='-') return false;
            }
        }
        return true;
